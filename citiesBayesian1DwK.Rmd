...
num_hypothetical_draws <- 15 # Number of times you simulate pilot study results
```
...
$$Y \sim N(\mu, \tau^2)$$
or is it that
$$Y_i \sim N(\theta_i,\sigma^2_i) \text{ and } \theta_i \sim N(\mu,\tau^2)$$
...
# Generate theta and sigmaSq based off of mu, tau, I
...
```{r, eval = FALSE}
updated_dat_generated <- list(I, Y=Y_P$mu, sigmaSq=Y_P$sd)
...
```
 ...
 Intuitively:
  - see if new prior switches which cities have positive impacts, or see if this changes which cities are in the highest C (num_pilots) number of cities
