...
basic_dat_generated <- list(J,I,Y=theta,sigmaSq=sigmaSq)
...
new_ranking <- function(fit_updated,Y_P) {
  ...
  # print("new rank")
  # print(new_rank)
  # Returns true if the first C elems of both vectors contain the same elements (unordered)
  return(new_rank)
}
```
...
change_mind <- function(K,Y,original_rank){
  ...
  print(new_rank)
  return(!setequal(original_rank[1:C],new_rank[1:C]))
}
```
...
for (i in 1:ncol(combinations)){
  K <- combinations[,i]
  nmc <- nmc + change_mind(K,Y,original_rank)
} 
nmc
```
